...
export const OPENCHAMBER_CONTROL_ACTION_DEFINITIONS = Object.freeze([
  ...
  { action: 'session.create', title: 'Create a session', description: 'Create a child session of the current session by default; prompt is optional. Set independent to true for a top-level session. Pass the same roleKey when continuing a logical child role so an idle, non-archived child is reused instead of creating a replacement' },
