...
- Physical-keyboard detection is native on both platforms: iOS reads `GCKeyboard`, and the Android `HardwareKeyboardPlugin` scans input devices, re-answers on config changes / foreground, and confirms on hardware key events. Both publish the same `oc:hardware-keyboard` contract the shared UI consumes, so the composer and draft layout start keyboard-aware instead of inferring it one focus late.
- Phones and tablets share one navigation model: a sessions drawer/sidebar on the left, the workspace drawer (Changes / Files / Terminal / Notes / MCP) on the right, and no overflow menu. When mobile bottom navigation is enabled, both sidebar action/tab bars move to the bottom; otherwise they retain their top placement. Tablets differ only in that the sessions list is a resizable persistent sidebar and the header dropdowns are anchored popovers.
- When mobile bottom navigation is enabled, both sidebar action/tab bars move to the bottom; otherwise they retain their top placement.
- Physical-keyboard detection is native on both platforms: iOS reads `GCKeyboard`, and the Android `HardwareKeyboardPlugin` scans input devices, re-answers on config changes / foreground, and confirms on hardware key events. Both publish the same `oc:hardware-keyboard` contract the shared UI consumes, so the composer and draft layout start keyboard-aware instead of inferring it one focus late.
